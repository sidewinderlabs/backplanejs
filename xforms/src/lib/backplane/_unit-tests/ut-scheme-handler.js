	...
	suite.add(
	  new YAHOO.tool.TestCase({
			...
			testFilePut: function () {
				...
				this.schemeHandler['PUT']('temp/test-file-put.txt', testString, 0, null);
				fileReader = document.fileIOFactory.createFileReader("temp/test-file-put.txt");
				Assert.areEqual(testString, fileReader.read(null, null, -1));
			},

			testFileGet: function () {
				var testStringIn = 'a string', testStringOut = '';
				this.schemeHandler['PUT']('temp/test-file-get.txt', testStringIn, 0, null);
				this.schemeHandler['GET']('temp/test-file-get.txt', null, 0,
					{
			...
			testFileDelete: function () {
				...
				this.schemeHandler["PUT"]("temp/test-file-delete.txt", "A test string", 0, null);
				this.schemeHandler["DELETE"]("temp/test-file-delete.txt", null, 0,
					{
						processResult: function (arg) {
							success = true;
						}
