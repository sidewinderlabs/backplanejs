...
function assignop(s,f){symbol(s,20).exps=true;return infix(s,function(left){var l;nonadjacent(prevtoken,token);nonadjacent(token,nexttoken);if(option.adsafe){l=left;do{if(predefined[l.value]===true){warning('ADsafe violation.',l);}
l=l.left;}while(l);}
...
function statements(begin){var a=[];if(begin&&nexttoken.type==='(string)'&&nexttoken.value.slice(0,10)==='use strict'){advance();advance(';');}
...
xmode=q;wmode=option.white;option.white=false;advance(q);statements(true);option.white=wmode;if(nexttoken.id!==q){error("Missing close quote on script attribute.");}
...
advance();}}}
switch(xmltype[xtype].doIt(n)){case'script':xmode='script';advance('>');indent=nexttoken.from;if(src){if(option.fragment&&option.adsafe){warning("ADsafe script violation.",token);}}else{statements(true);}
...
return syntax['function'];});syntax['new'].exps=true;infix('.',function(left){adjacent(prevtoken,token);var t=this,m=identifier();if(typeof m==='string'){countMember(m);}
...
if(option.adsafe){for(;;){if(banned[m]===true){warning("ADsafe restricted word '{a}'.",token,m);}
...
this.left=left;return this;},155).exps=true;prefix('(',function(){nospace();var v=parse(0);advance(')',this);nospace(prevtoken,token);if(v&&(v.id==='function'||v.id==='.'||v.id==='['||(v.id==='('&&v.left&&v.left.id==='function'))){warning("Parens are not needed here.",this);}
return v;}).fud=function(){nospace();var v=parse(0);advance(')',this);if(v&&v.id==='function'){advance('(');return token.led(v);}
...
if(o.adsafe){o.browser=false;o.debug=false;o.eqeqeq=true;o.evil=false;o.forin=false;o.glovar=true;o.nomen=true;o.on=false;o.rhino=false;o.sidebar=false;o.undef=true;o.widget=false;predefined.Date=false;predefined['eval']=false;predefined.Function=false;predefined.Object=false;}
option=o;}else{option={};}
option.indent=option.indent||4;tab='';for(i=0;i<option.indent;i+=1){tab+=' ';}
indent=0;global=Object.beget(predefined);scope=global;funct={'(global)':true,'(name)':'(global)','(scope)':scope,'(breakage)':0,'(loopage)':0};functions=[];src=false;xmode=false;xtype='';stack=null;member={};membersOnly=null;implied={};inblock=false;lookahead=[];jsonmode=false;warnings=0;lex.init(s);prereg=true;prevtoken=token=nexttoken=syntax['(begin)'];assume();try{advance();if(nexttoken.value.charAt(0)==='<'){xml();}else if(nexttoken.id==='{'||nexttoken.id==='['){option.laxbreak=true;jsonmode=true;jsonValue();}else{statements(true);}
...
{var fso=WScript.CreateObject("Scripting.FileSystemObject");var itself=function()
{};function addRescanLink(oStream,sType,sIn,sOut)
{oStream.WriteLine("<script>function rescan(sLintLoc,sType,sScriptLoc,sOut){ var oShell = new ActiveXObject('WScript.Shell'); if(oShell) {  oShell.Run('cscript.exe '+  sLintLoc + ' ' +sType+' '  + sScriptLoc + ' ' + sOut,5,true);  window.location.reload(); }}</script>");oStream.WriteLine("<a href=\"javascript:rescan('"+String(WScript.ScriptFullName).replace(/\\/g,'\\\\')+"','"+sType+"','"+sIn.replace(/\\/g,'\\\\')+"','"+sOut.replace(/\\/g,'\\\\')+"')\">Rescan?</a>");}
function lintFolders(oFolders,oFolderOut,oSummaryStream)
{var errorCount=0;var FoldersEnum=new Enumerator(oFolders);for(;!FoldersEnum.atEnd();FoldersEnum.moveNext())
{var oFolderIn=FoldersEnum.item();if(!(oFolderIn.Attributes&2)){if(oFolderIn.Files.Count||oFolderIn.SubFolders.Count){var sFolderSpec=oFolderOut.Path+"\\"+oFolderIn.Name;var dOut;if(fso.FolderExists(sFolderSpec)){dOut=fso.GetFolder(sFolderSpec);}
else{dOut=fso.CreateFolder(sFolderSpec);}
var folderErrors=itself.lintDir(oFolderIn,dOut);errorCount+=folderErrors;oSummaryStream.WriteLine("<tr  class='"+((folderErrors>0)?"erroneous":"clean")+"'><td><a href='"+oFolderIn.Name+"/dir.lnt.htm'>/"+oFolderIn.Name+"</a></td><td>"+oFolderIn.Files.Count+"</td><td>"+folderErrors+"</td></tr>");}}}
return errorCount;}
function lintFiles(oFiles,oFolderOut,oSummaryStream)
{var errorCount=0;var fileErrorCount=0;var srcFiles=new Enumerator(oFiles);var thisFile;oSummaryStream.WriteLine("<table><tr><th>file</th><th>errors</th></tr>");for(;!srcFiles.atEnd();srcFiles.moveNext()){thisFile=srcFiles.item();if(thisFile.Type==="JScript Script File"){fileErrorCount=itself.lintFile(thisFile,oFolderOut);errorCount+=fileErrorCount;oSummaryStream.WriteLine("<tr class='"+((fileErrorCount>0)?"erroneous":"clean")+"'><td><a href='"+thisFile.Name+".lnt.htm'>"+thisFile.Name+"</a></td><td> "+fileErrorCount+"</td></tr>");}}
oSummaryStream.WriteLine("</table>");return errorCount;}
itself.lintDir=function(oFolderIn,oFolderOut)
{var errorCount=0;WScript.StdOut.WriteLine("Linting Folder: "+oFolderIn.Name);var oStreamOut=oFolderOut.CreateTextFile("dir.lnt.htm",true);if(oStreamOut)
{oStreamOut.WriteLine("<style>.erroneous{background-color:#ff9999}.clean{background-color:#33ffcc}</style>");errorCount+=lintFiles(oFolderIn.Files,oFolderOut,oStreamOut);oStreamOut.WriteLine("<table><tr><th>folder</th><th>files</th><th>errors</th></tr>");oStreamOut.WriteLine("<tr class='"+((errorCount>0)?"erroneous":"clean")+"'><td>.</td><td>"+oFolderIn.Files.Count+"</td><td>"+errorCount+"</td></tr>");errorCount+=lintFolders(oFolderIn.SubFolders,oFolderOut,oStreamOut);oStreamOut.WriteLine("</table>");oStreamOut.WriteLine("Total Lint Errors: "+errorCount);oStreamOut.WriteLine("<br/><a href='../dir.lnt.htm'>..</a>");addRescanLink(oStreamOut,"d",String(oFolderIn),String(oFolderOut));oStreamOut.Close();}
return errorCount;};itself.lintFile=function(oFile,oFolderOut)
{var errorCount=0;if(oFile.Type==="JScript Script File")
{var oStreamIn=oFile.openAsTextStream(1);WScript.StdOut.WriteLine("Linting: "+oFile.Name);var oStreamOut=oFolderOut.CreateTextFile(oFile.Name+".lnt.htm",true);oStreamOut.Write("<a href='dir.lnt.htm'>DIR</a> | ");addRescanLink(oStreamOut,"f",String(oFile),String(oFolderOut));if(!JSLINT(oStreamIn.ReadAll(),{passfail:false}))
{errorCount=JSLINT.errors.length;oStreamOut.WriteLine("<br /> Lint Errors:"+errorCount+"<br/>");oStreamOut.Write(JSLINT.report());}
else
{oStreamOut.WriteLine("<br /> This file is free of (lint-detected) errors!<br/>");oStreamOut.Write(JSLINT.report());}
oStreamOut.Close();oStreamIn.Close();}
return errorCount;};return itself;}();(function(){if(WScript.Arguments.Length===0)
{WScript.Echo("cscript.exe jslint.js [d|f] in out\n\n"+"\td \t (default) run jslint on the contents of a directory (deep)\n"+"\tf \t run jslint on a single file\n"+"\tin \t the file or directory on which to run jslint.\n"+"\tout \t the directory in which to store the jslint output.\n"+"\nEach .js file in the input will have a corresponding .lnt.htm file in the output directory\n"+"The output directory's structure will match that of the input directory");WScript.Quit(0);}
var sFileOrDir=String(WScript.Arguments.Item(0)).toLowerCase();var bLintDir=sFileOrDir==="d";var bLintFile=sFileOrDir==="f";var argIx1=1;var argIx2=2;if(!bLintDir&&!bLintFile)
{argIx1=0;argIx2=1;bLintDir=true;}
var fso=WScript.CreateObject("Scripting.FileSystemObject");var oOutFolder=fso.GetFolder(WScript.Arguments.Item(argIx2));if(bLintFile)
{var oFile=fso.GetFile(WScript.Arguments.Item(argIx1));FileSystemJSLINT.lintFile(oFile,oOutFolder);}
else
{var oInFolder=fso.GetFolder(WScript.Arguments.Item(argIx1));FileSystemJSLINT.lintDir(oInFolder,oOutFolder);}})();